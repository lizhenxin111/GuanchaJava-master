...
class UserPraiseFragment : BaseListFragment<UserArticleAdapter>() {
    ...
    companion object {
        fun newInstance(url: String): UserPraiseFragment = UserPraiseFragment().apply {
            arguments = Bundle().apply {
                putString(TAG.TAG_FRAGMENT_NEWS_LIST_URL, url)
            }
        }

        fun newInstance(url: String, loadNow: Boolean): UserPraiseFragment = UserPraiseFragment().apply {
            arguments = Bundle().apply {
                putString(TAG.TAG_FRAGMENT_NEWS_LIST_URL, url)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mNewsListViewModel.userPraise.observe(viewLifecycleOwner, Observer {
    ...
    override fun requestData(adapter: UserArticleAdapter, swipe: SwipeRefreshLayout) {
        mNewsListViewModel.getUserPraise(mApi + 1)
    }

    override fun requestMore(mAdapter: UserArticleAdapter) {
        mNewsListViewModel.getUserPraise(mApi + mPageNo++)
    }
